...
//				logger.info("---计费定时器正在执行---");
					...
					// 如果足够 继续聊天
					// 否则断开链接 并扣除用户的金币
					logger.info("用户{}和{}聊天,金币{},每分钟金币消耗{},聊天时长{},房间号{}", k,v.get("anthorId"), v.get("gold"),v.get("deplete"),v.get("timing"),v.get("roomId"));
 
					if (v.get("timing") < 60 ) {
						// 聊天时间增加1秒
						v.put("timing", v.get("timing") + 1);
					/**
					 *  时间刚好60秒钟 且用户金币只够聊天1分钟  加入到需要清理的集合中
					 */
					...
					//用户金币大于1分钟的聊天时间
						...
						if(v.get("gold") < v.get("deplete")*((timing==0?1:timing)+1) && !arr.contains(k)) {
							logger.info("开始给{}推送金币不足",v.get("userId"));
						    IoSession session = UserIoSession.getInstance().getMapIoSession(v.get("userId"));
		...
		try {
			for (Map.Entry<Integer, Map<String, Integer>> map : clearUser.entrySet()) {
				...
				if (null == room) {
					...
					room.setChatType(map.getValue().get("chatType"));
					room.setConversationType(map.getValue().get("conversationType"));
					RoomTimer.useRooms.put(map.getValue().get("roomId"), room);
				}
				// 清除当前已计算了结果了的
				clearUser.remove(map.getKey());
				//判断是否在延迟计时中 如果在那么删除数据
				if(SocketDelayTimer.socketDelayUser.containsKey(map.getKey())) {
					SocketDelayTimer.socketDelayUser.remove(map.getKey());
				}
				//挂断连接
				videoChatService.breakLink(room.getRoomId(), 5, room.getLaunchUserId());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Room getRoom(int roomId) {
		...
		for (Map.Entry<Integer, Map<String, Integer>> map : timingUser.entrySet()) {
			...
			if (null != map.getValue() && map.getKey() == roomId) {
				room = new Room(map.getValue().get("roomId"));
				room.setLaunchUserId(map.getValue().get("userId"));
				room.setChatType(map.getValue().get("chatType"));
				room.setCoverLinkUserId(map.getValue().get("anthorId"));
			}
		}
	...
	public static boolean getUserExist(int userId) {
		for (Map.Entry<Integer, Map<String, Integer>> map : timingUser.entrySet()) {
			LoggerFactory.getLogger(VideoTiming.class).info("map中的数据-->{}",map.toString());
			if (null != map.getValue() && map.getValue().get("anthorId") == userId ||map.getValue().get("userId") == userId) {
				return true;
			}
		}
		return false;
	 ...
	 * @param userId
	 */
	public static Map<String, Integer> getUserMoneyMap(int userId) {
		for (Map.Entry<Integer, Map<String, Integer>> map : timingUser.entrySet()) {
			LoggerFactory.getLogger(VideoTiming.class).info("map中的数据-->{}",map.toString());
			if (null != map.getValue() && map.getValue().get("anthorId") == userId || map.getValue().get("userId") == userId) {
				return map.getValue();
	...
	public static List<Integer> getByUserResRoom(int userId){
		...
		for (Map.Entry<Integer, Map<String, Integer>> map : timingUser.entrySet()) {
            //根据用户编号返回房间号集合
			if (map.getValue().get("anthorId") == userId || map.getValue().get("userId") == userId) {
